...
(function () {
  ...
  let re1 = /\d\w+/,
    re2 = /([a-fA-F0-9]{32})/,
    re3 =/\.[0-9a-z]+$/i;
  let REyandeResult = REyande.test(originUrl);
  let REkonachanResult = REkonachan.test(originUrl);
  let REdanbooruResult = REdanbooru.test(originUrl);
  let parse = null
  ...
  class BooruDownloader {
    ...
    initMenuPanel() {
         ...
         <div class="board-header">
          ...
          <div class="board-content">
            ...
            <div class="board-content-row">
              ...
              <a class="row-content hover-item-line" href="https://greasyfork.org/zh-CN/scripts/371605-booru-selector-downloader" target="_blank">v2.0.0</a>
            </div>
      ...
      this.loadCssCode(`:root { --primary-backgroundColor: #eee; --primary-lineBackgroundColor: #ccc; --primary-fontColor: #ee8887; --primary-fontColorHover: #ff4342; --primary-headerFontColor: #ffffff; --primary-border: 1px solid transparent; } [data-theme=light] .darkToggleIcon { display: none; } [data-theme=dark] .lightToggleIcon { display: none; } ul#post-list-posts li { float: none } .imgItem:hover, .imgItem:focus { outline: 1px solid var(--primary-fontColor); } .imgItemChecked { outline: 1px solid var(--primary-fontColor); } article.post-preview { float: none; } .helper-board { width: 450px; height: 320px; position: fixed; font-size: 12px; right: 10px; bottom: 0; background: var(--primary-backgroundColor); color: var(--primary-fontColor); border: var(--primary-border); border-radius: 5px; overflow: hidden; transition: all cubic-bezier(.22,.58,.12,.98) .4s; box-shadow: 0 2px 12px 0 rgba(246, 150, 149, 0.6); } .helper-board.helper-board-small { width: 50px; height: 50px; border-radius: 50%; bottom: 50vh; cursor: pointer; background: var(--primary-fontColor); } .helper-board.helper-board-small .board-header { width: 50px; height: 50px; border-radius: 50%; } .helper-board.helper-board-small .board-header .board-close-button, .helper-board.helper-board-small .board-header .board-header-text, .helper-board.helper-board-small .theme-btn { display: none; } .helper-board.helper-board-small .board-header .board-header-small-tip { display: block; width: 50px; height: 50px; border-radius: 50%; line-height: 46px; text-align: center; font-size: 26px; } .board-header { height: 30px; color: var(--primary-headerFontColor); background: var(--primary-fontColor); font-size: 16px; } .board-header-inner { display: flex; align-items: center; justify-content: space-between; padding-right: 32px; } .board-content { overflow-y: auto; overflow-x: hidden; max-height: 340px; padding: 10px 12px; height: 100%; font-size: 14px; } .board-header-text { line-height: 30px; padding-left: 10px; font-size: 14px; } .board-close-button { position: absolute; top: 5px; right: 3px; width: 20px; height: 20px; margin: 0; padding: 0; cursor: pointer; transition: all .3s; } .board-close-button:hover { color: var(--primary-headerFontColor); } .board-header-small-tip { display: none; } .board-content-row { display: flex; align-items: center; margin-bottom: 8px; height: 20px; } .row-label { color: var(--primary-fontColor); margin-right: 8px; } .row-content { display: flex; align-items: center; } .hover-item-line { color: var(--primary-fontColor); text-decoration: underline!important; cursor: pointer; } .hover-item-line:hover { color: var(--primary-fontColorHover); } .hover-item { color: var(--primary-fontColor); cursor: pointer; } .hover-item:hover { color: var(--primary-fontColorHover); } .download-row-container { margin-top: 15px; } .download-row { display: flex; align-items: center; margin-bottom: 5px; } .download-row-title { color: var(--primary-fontColor); margin-right: 10px; } .download-row-line { flex: 1; height: 4px; background: var(--primary-lineBackgroundColor); margin-right: 10px; } .download-row-line-active { height: 100%; background: var(--primary-fontColor); transition: width .4s ease; } .download-row-percent { color: var(--primary-fontColor); width: 50px; } .fav-state { margin-left: 7px; color: var(--primary-fontColor); } .re-try-icon svg { margin-left: 5px; cursor: pointer; transform: translateY(3px); } .theme-btn { background: none; border: none; color: var(--primary-headerFontColor); cursor: pointer; font-family: inherit; padding: 0; align-items: center; border-radius: 50%; display: flex; height: 100%; justify-content: center; }`)
      this.initMenuEvent()
    }
    ...
    async handleDownLoadImg(e, type = 'larger') {
      ...
      for (let i = 0; i < postsItems.length; i++) {
        const link = postsItems[i].getElementsByClassName('thumb')[0].href
        const id = Number(postsItems[i].getAttribute('id').replace('p', ''))
        const imgInfo = await this.fetchDetailPage(link)
        imgs.push({
          id,
          url: imgInfo[type],
    ...
    downloadHandler(img) {
      return new Promise((resolve, reject) => {
        const arg = {
          ...
          name: img.fileName,
          onprogress: (xhr) => {
            this.downloadProgress(xhr, img.id)
        ...
        if(!downloadRowEl) {
          ...
          const template = `
            <div id="download-row-${img.id}" class="download-row">
               <div class="download-row-title">${img.id}</div>
               <div class="download-row-line">
                 <div class="download-row-line-active" style="width: 0%;"></div>
               </div>
      ...
      if(!downloadRowEl) {
        ...
        const template = `
            <div id="download-row-${id}" class="download-row">
               <div class="download-row-title">${id}</div>
               <div class="download-row-line">
                 <div class="download-row-line-active" style="width: 0%;"></div>
               </div>
               <div class="download-row-percent">0%</div>
    ...
    downloadProgress(xhr, id) {
      try {
        if (!xhr.lengthComputable) return;
        let width = Math.floor(xhr.loaded / xhr.total * 100);
        const downloadRowEl = $(`#download-row-${id}`)
        if(!downloadRowEl) {
          // create downloadRow
          const downloadList = $('.download-row-container')
    ...
    fetchFavorite(id, isLike) {
      if(REyandeResult) {
        ...
        const pageUrl = `https://yande.re/post/show/${id}`
        fetch("https://yande.re/post/vote.json", {
          "headers": {
     ...
     * @param   {string}      log.pageUrl
     * @param   {string}      log.result    success | error
     * */
    addFavoriteLog(log) {
      const { id, isLike, pageUrl, result } = log
    ...
    fetchDetailPage(link) {
      return new Promise((resolve, reject) => {
        ...
        if(REdanbooruResult) {
          largerReg = new RegExp('Size: <a href="[^\"]+');
          fetch(link)
            .then(res => res.text())
            .then(res => {
              const bodyText = res
              const src = largerReg.exec(bodyText)[0].replace('Size: <a href="', '')
              console.log(imgInfo);
              resolve(imgInfo)
            }).catch(e => {
            console.log(e)
